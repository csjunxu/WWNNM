function [E_Img]   =  FLRAD_DeNoising( N_Img, O_Img, Par )
...
for iter = 1 : Par.Iter        
    ...
    if (mod(iter-1,Par.Innerloop)==0)                                           % 如果iter-1是Innerloop的整数倍的话mod=0
        NL_mat  =  Block_matching(CurPat, Par, Neighbor_arr, Num_arr, Self_arr,Sigma_arr);% Caculate Non-local similar patches for each 
        if(iter==1)
