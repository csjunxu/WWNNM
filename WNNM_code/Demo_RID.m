...
SSIM = [];
for i = 1:im_num
    O_Img = im2double( imread(fullfile(GT_Original_image_dir, GT_im_dir(i).name)) );
    N_Img = im2double(imread(fullfile(TT_Original_image_dir, TT_im_dir(i).name) ));
    nSig = NoiseEstimation(N_Img * 255, 6);
    fprintf( 'Noisy Image: nSig = %2.3f, PSNR = %2.2f \n\n\n', nSig, csnr( N_Img * 255, O_Img * 255, 0, 0 ) );
    Par   = ParSet(nSig);
...
name = sprintf(['../WNNM_NL_RID_CC_' num2str(im_num) '_nSig' num2str(nSig) '.mat']);
